// Prueba visual CDP de 6 de los 7 casos resueltos del lote 11.
...
(async () => {
  ...
  const CASOS = [
    ['Nopales con huevo', /10 min y escúrrelos \(preparación: Ensalada de nopales con queso\)/],
    ['Carne molida con nopales', /10 min y escúrrelos \(preparación: Ensalada de nopales con queso\)/],
    ['Crema de brócoli con queso', /al vapor 5 min y licúalo con leche \(preparación: Verduras cocidas al vapor con limón y chile\)/],
    ['Rajas con crema y queso', /comal seco 8 min, hasta que la piel quede negra y ampollada/],
    ['Chiles rellenos de picadillo', /Asa los 2 chiles poblanos enteros como en Rajas con crema \(técnica\)/],
    ['Pozole verde', /hasta que truenen — 4 min \(preparación: Pollo en pipián verde\)/]
  ];
  for (const [nombre, rx] of CASOS) {
    await abrir(await idxDe(nombre));
    if (nombre === 'Pozole verde') { await evalJs("(function(){var b=[].slice.call(document.querySelectorAll('#recetaDetalle button')).find(function(x){return /Siguiente/.test(x.textContent);}); if(b)b.click();})()"); }
